	...
	<view class="page_box">
		...
		<view class="content_box">
			<scroll-view scroll-y="true" @scrolltolower="loadMore" class="scroll-box">
				<u-checkbox-group @change="checkboxGroupChange" activeColor="#e9b461">
					<view class="collect-list u-flex u-flex-1" v-for="(f,index) in browseList" :key="index">
						<u-checkbox v-if="isEdit" :name="index" shape="circle" v-model="f.checked"></u-checkbox>
						<shopro-mini-card :image="f.image_path" :title="f.title" :price="toFixed2(f.price*f.discount)"
							:originPrice="f.price" :subtitle="f.subtitle" :tag="'民宿'"
							@click="$Router.push({ path: '/pages/goods/detail', query: { id: f.goods_id,dataType:f.goods_type } })">
						</shopro-mini-card>
					</view>
				</u-checkbox-group>
				<!-- 缺省页 -->
				<shopro-empty v-if="isEmpty" :image="$IMG_URL + '/imgs/empty/empty_goods.png'" tipText="暂无浏览记录"
					btnText="去首页逛逛" @click="$Router.pushTab('/pages/index/index')"></shopro-empty>
				<!-- 更多 -->
		...
		<view class="foot_box ">
			<view class="tools-box u-flex u-row-between u-flex-1" v-show="isEdit && !isEmpty">
				<u-checkbox shape="circle" activeColor="#e9b461" @change="onAllSel" v-model="isAllSel"><text>全选</text>
				</u-checkbox>

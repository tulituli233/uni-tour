...
<template>
	<view class="container">
		...
		<view class="user-list u-flex u-row-between">
			<text class="list-name">城市</text>
			<view class="u-flex">
				<input class="list-val" v-model="userData.city" @input="onChangeCity" @blur="onChangeCity"
					@confirm="onChangeCity" placeholder="请输入所在城市~" maxlength="50"
					placeholder-style="color:#c8c9cc;fontSize:28rpx" />
				<text class="u-iconfont uicon-arrow-right u-m-l-20" style="color:#999;"></text>
			</view>
		</view>

		<view class="user-list u-flex u-row-between" style="height: 240rpx;">
			<text class="list-name">简介</text>
			<textarea v-model="userData.introduction" cols="30" maxlength="255" @input="onChangeIntroduction"
				@blur="onChangeIntroduction" @confirm="onChangeIntroduction" class="introductionText"
				rows="3">{{userData.introduction==''?'请输入简介~':userData.introduction}}</textarea>
			<!-- <view>
				<input v-model="userData.introduction" @input="onChangeNickName"
					@blur="onChangeNickName" @confirm="onChangeNickName" placeholder="请输入简介~" maxlength="50"
					type="textarea" rows="3" placeholder-style="color:#c8c9cc;fontSize:28rpx" />
				<text class="u-iconfont uicon-arrow-right u-m-l-20" style="color:#999;"></text>
			</view> -->
		</view>
		...
		<view class="user-list u-flex u-row-between" @tap="bindMobile">
			...
			<view class="u-flex">
				<text class="list-val">{{ userInfo.phone_number || "暂未绑定手机号" }}</text>
	...
	export default {
		...
		data() {
			return {
				...
				showCalendar: false,
				showModal: false,
				editInfoDisabled: true, //保存按钮是否禁用
		...
		methods: {
			...
			// 修改所在城市
			onChangeCity() {
				this.editInfoDisabled = this.userData.city == this.userInfo.city;
			},
			// 修改简介
			onChangeIntroduction() {
				// console.log('this.userInfo.introduction', this.userInfo.introduction);
				// console.log('this.userData.introduction', this.userData.introduction);
				this.editInfoDisabled = this.userData.introduction == this.userInfo.introduction;
			},
			// 修改手机号
			bindMobile() {
				this.$u.toast('该功能尚未开放,敬请期待!');
			...
			editUserInfo() {
				...
				that.$http(
					"user.reUserInfo", {
						...
						introduction: that.userData.introduction,
						// birthday: that.userData.birthday,
						// avatar: that.userData.fileUrl
					},
					"保存中..."
				).then(res => {
					console.log('user.reUserInfo.res', res);
					if (res.code === 200) {
						// let token = uni.getStorageSync('token')
						that.getUserInfo({
							id: that.userData.user_id
						});

...
<template>
	<view class="page_box">
		<view class="head_box u-flex">
			<button :class="{ 'btn-active': typeCurrent === t.code }" class="u-reset-button type-btn"
				@tap="selType(t.code)" v-for="t in commentTypeList" :key="t.code">
				{{ t.name }}({{ t.num }})
			</button>
		</view>
		<view class="content_box">
			<scroll-view scroll-y="true" enable-back-to-top @scrolltolower="loadMore" class="scroll-box">
				...
				<shopro-empty v-if="isEmpty" :image="$IMG_URL + '/imgs/empty/comment_empty.png'" tipText="暂无此类评价~">
				</shopro-empty>
				<!-- 加载更多 -->
		...
		methods: {
			...
			getCommentType() {
				...
				that.$http('goods.commentType', {
					...
					console.log('goods.commentType', res);
					if (res.code === 1) {
			...
			getCommentList() {
				...
				that.$http('goods.commentList', {
					...
					console.log('goods.commentList', res);
					if (res.code === 1) {

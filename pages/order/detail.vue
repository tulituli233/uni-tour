	...
	<view class="order-detail-wrap">
		...
		<view class="detail-goods">
			...
			<view class="order-list">
				<view class="order-card" @tap="jump('/pages/goods/detail', { id: orderDetail.goods_id,dataType:orderDetail.goods_type })">
					<shopro-mini-card :title="orderDetail.title" :image="orderDetail.image_path">
						<template #describe>
							<view class="order-sku u-ellipsis-1">
								<text class="order-num">数量:{{ orderDetail.num || 0 }};</text>
								{{ orderDetail.goods_sku_text ? orderDetail.goods_sku_text : '' }}
							</view>
						</template>
						<template #cardBottom>
							<view class="card-price-box u-flex">
								<text class="order-price font-OPPOSANS">￥{{ toFixed2(orderDetail.price*orderDetail.discount) || 0 }}</text>
								<button class="u-reset-button status-btn"
									v-if="orderDetail.status_name">{{ orderDetail.status_name }}</button>
							</view>
						</template>
					</shopro-mini-card>
				</view>
		...
		<!-- 订单信息 -->
		<view class="notice-box" v-if="orderDetail.orders_id">
			<view class="notice-box__content">
				<view class="notice-item--center u-flex">
					<text class="title">订单编号：</text>
					<text class="detail">{{ orderDetail.orders_id }}</text>
					<button class="u-reset-button copy-btn" @tap="onCopy(orderDetail.orders_id)">复制</button>
				</view>
				<view class="notice-item u-flex">
					<text class="title">下单时间：</text>
					<text class="detail">{{ orderDetail.createtime }}</text>
				</view>
				<view class="notice-item u-flex" v-if="orderDetail.order_type > 0">
		...
		<!--  价格信息 -->
		<view class="order-price-box u-m-b-20" v-if="orderDetail.orders_id">
			<view class="notice-item u-flex u-row-between">
				<text class="title">商品总额</text>
				<text class="detail font-OPPOSANS">￥{{ toFixed2(orderDetail.price*orderDetail.discount) }}</text>
			</view>
			<!-- <view class="notice-item u-flex u-row-between">
			...
			<view class="notice-item u-flex u-row-between">
				...
				<text class="detail font-OPPOSANS">-￥{{ orderDetail.discount_fee || 0}}</text>
			</view>
			...
			<view class="notice-item all-rpice-item u-flex" style="width: 100%;">
				<text class="title">{{ orderDetail.order_type <= 0 ? '需付款' : '实付款' }}：</text>
				<text class="detail all-price font-OPPOSANS">￥{{ toFixed2(orderDetail.price*orderDetail.discount) }}</text>
			</view>
		</view>
			...
			<view class="foot_box safe-area-inset-bottom">
				...
				<view class="btn-box u-flex">
					<view class="" v-for="btn in orderDetail.btns" :key="btn">
						<button v-if="btn === 'cancel'" @tap.stop="onCancel(orderDetail.id)"
							class="u-reset-button obtn1">取消订单</button>
						<button v-if="btn === 'pay'" @tap.stop="onPay(orderDetail.id)"
							class="u-reset-button obtn2">付款</button>
						<button v-if="btn === 'groupon'"
							@tap.stop="jump('/pages/activity/groupon/detail', { id: orderDetail.ext_arr.groupon_id })"
			...
			getOrderDetail() {
				...
				that.$http('order.detail', {
					...
					if (res.code === 200) {
						...
						that.orderDetail.createtime = that.$u.timeFormat(res.data.created_at, 'yyyy-mm-dd hh:MM:ss');
						that.orderDetail.paytime = that.$u.timeFormat(res.data.created_at, 'yyyy-mm-dd hh:MM:ss');
						// that.orderDetail.createtime = res.data.created_at
						// that.orderDetail.paytime = res.data.created_at
					}
				});
			},
			// 复制

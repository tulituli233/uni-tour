	...
	<view class="page_box">
		...
		<view class="content_box">
			<scroll-view scroll-y="true" enable-back-to-top @scrolltolower="loadMore" class="scroll-box">
				<!-- 订单列表 -->
				<view class="order-list" v-for="(order, orderIndex) in orderList" :key="order.orders_id"
					@tap.stop="jump('/pages/order/detail', { id: order.orders_id,dataType:order.goods_type })">
					<view class="order-head u-flex u-row-between">
						<text class="no">订单编号：{{ order.orders_id }}</text>
						<text class="state">{{ orderState[order.order_type+1].title }}</text>
					</view>
					<view class="goods-order">
						<view class="order-content">
							<shopro-mini-card :title="order.title" :image="order.image_path">
								<template #describe>
									<view class="order-sku u-ellipsis-1">
										<text class="order-num">数量:{{ order.num || 0 }};</text>
										{{ order.title ? order.title : '' }}
									</view>
								</template>
								<template #cardBottom>
									<view class="order-price-box u-flex ">
										<text class="order-price font-OPPOSANS">￥{{ order.price || 0 }}</text>
										<button class="u-reset-button status-btn"
											v-if="order.order_type">{{ 111 }}</button>
									</view>
								</template>
							</shopro-mini-card>
						</view>
					...
					<view class="order-bottom">
						<view class="all-msg u-flex font-OPPOSANS">
							...
							{{ order.discount }} ，原价：
							<text class="all-unit">￥</text>
							{{ order.price }} ，{{ order.order_type <= 0 ? '需付款' : '实付款' }}：
							<view class="all-money font-OPPOSANS">{{ toFixed2(order.price*order.discount) }}</view>
						</view>
	...
	export default {
		...
		methods: {
			...
			getOrderList() {
				...
				that.$http('order.orderList', {
					// that.$http('user.noteList', {
					type: that.orderType,
					page: that.currentPage,
					user_id: userInfo.user_id
				}, '加载中...').then(res => {
					console.log('order.orderList.res', res);
					if (res.code === 200) {

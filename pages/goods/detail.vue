	...
	<view class="detail-wrap ">
		...
		<view class="detail_box" v-else>
			<view class="detail-content">
				<!-- 轮播 -->
				<u-swiper :height="750" borderRadius="0" :list="images" indicator-pos="bottomRight" mode="number"
					:interval="3000" @click="onGoodsSwiper"></u-swiper>

				<!-- <image :src="goodsInfo.image_path" mode=""></image> -->

				<!-- 价格卡片组 -->
				<sh-price-card v-if="goodsInfo.homestay_id" :detail="goodsInfo" :type="detailType"
					@change="getActivityRules">
				</sh-price-card>

				<!-- 标题 -->
				<view class="u-m-b-10 title-box u-p-20 u-skeleton-fillet">
					<view class="goods-title u-m-b-10 u-ellipsis-2">{{ goodsInfo.homestay_name }}</view>
					<view class="sub-title u-ellipsis-2">{{ goodsInfo.merchant_name }}</view>
				</view>
				...
				<!-- 规格选择 -->
				<!-- <view class="sku-box" @tap="showSku = true" v-if="showSku">
				...
				<view class="tab-detail u-p-20 u-m-b-10">
					...
					<view class="rich-box " v-if="tabCurrent === 'tab0'">
						<u-parse :html="goodsInfo.introduction"></u-parse>
					</view>
					<!-- 参数 -->
					...
					<view class="rich-box " v-if="tabCurrent === 'tab3'">
						<u-parse :html="goodsInfo.introduction"></u-parse>
					</view>
				</view>
			</view>
			...
			<!-- 商品foot -->
			<view class="tabbar-foot safe-area-inset-bottom" v-if="goodsInfo.homestay_id">
				<view class="detail-foot_box safe-area-inset-bottom  u-flex u-col-ceter u-row-around">
					...
					<view class="left u-flex">
						...
						<view class="tools-item u-flex-col u-row-center u-col-center" @tap="goHome">
							<image class="tool-img shopro-selector-circular" src="/static/images/tabbar/home1.png"
								mode=""></image>
						...
						<block v-if="detailType !== 'score'">
							<view class="tools-item u-flex-col u-row-center u-col-center"
								@tap="onFavorite(goodsInfo.homestay_id)">
								<image class="tool-img"
									:src="favorite ? $IMG_URL + '/imgs/detail/detail_favorite_end.png' : $IMG_URL + '/imgs/detail/detail_favorite.png'"
									mode=""></image>
					...
					<view class="detail-right">
						...
						<view v-if="detailType !== 'score'">
							...
							<view class="detail-btn-box u-flex u-row-around" v-if="!goodsInfo.activity_type">
								...
								<button class=" u-reset-button tool-btn pay-btn" @tap="showModal = true">立即预定</button>
							</view>
							<!-- 活动按钮 -->
			...
			<!-- 规格弹窗 -->
			<!-- <shopro-sku
				v-if="showSku && goodsInfo.homestay_id"
				v-model="showSku"
				:goodsInfo="goodsInfo"
			...
			<!-- 登录 -->
			<tour-auth-modal v-if="authType"></tour-auth-modal>
			<!-- modal -->
			<u-modal ref="uModal" v-model="showModal" :show-cancel-button="true" confirm-color="#F54141"
				cancel-color="#666666" @confirm="goPay" content="确定预定吗？"></u-modal>
		</view>
	</view>
</template>
	...
	export default {
		...
		data() {
			return {
				...
				images: ['http://file.shopro.top/uploads/20210519/6a9b21c6158e9b429f961b810ac0730b.jpg',
					'http://file.shopro.top/uploads/20210519/6a9b21c6158e9b429f961b810ac0730b.jpg'
				],
				dataType: 1,
				userInfo: {},
				showModal: false,
		...
		async onLoad() {
			...
			that.dataType = this.$Route.query.dataType;
		...
		methods: {
			...
			// 轮播图预览
			onGoodsSwiper(e) {
				this.$tools.previewImage(this.images, e);
			},
			// 获取活动规则
			getActivityRules(e) {
				if (e) {
			...
			getScoreDetail() {
				...
				return new Promise((resolve, reject) => {
					console.log('that.$Route.query.id', that.$Route.query);
			...
			getGoodsDetail() {
				...
				return new Promise((resolve, reject) => {
					that.$http(
						'goods.detail', {
							id: that.$Route.query.id,
							dataType: that.$Route.query.dataType,
						},
						'',
						false
					).then(res => {
						console.log('goods.detail.res', res);
						if (res.code === 200) {
							...
							// 	}
							// });
							// that.getCommentList();image_path

							// 增加浏览记录
							that.addNote()
							// 获取图片数组
							// that.images = res.data.images.split(',')
							that.getImages(res.data.images)
							// 判断收藏
							that.isFavorite()
							resolve(res.data);
						} else {
			...
			// 增加浏览记录
			addNote(goods_id, image_path) {
				let that = this;
				let user_id = that.userInfo.user_id
				that.$http('user.addNote', {
					user_id,
					goods_id: this.goodsInfo.homestay_id,
					goods_type: that.dataType,
					image_path: this.goodsInfo.image_path,
					title: this.goodsInfo.homestay_name,
					subtitle: this.goodsInfo.merchant_name,
					price: this.goodsInfo.price,
					discount: this.goodsInfo.discount,
				}).then(res => {
					console.log('user.addNote', res);
					if (res.code === 200) {
						console.log('user.addNote——OK');
					}
				});
			},
			// 获取图片数组
			getImages(images) {
				...
				this.images = arr.map(item => {
					return 'http://127.0.0.1:7777/uploads/images/' + item
				})
				// let that = this;
				// that.$http('goods.images', {
				// 	images
				// }).then(res => {
				// 	console.log('goods.images', res);
				// 	// console.log('ROUTES', that.$Router);
				// 	if (res.code === 200) {
				// 		that.images = that.setImages(res.data)
				// 	} else {
				// 		let images = [
				// 			'http://file.shopro.top/uploads/20210519/6a9b21c6158e9b429f961b810ac0730b.jpg',
				// 			'http://file.shopro.top/uploads/20210519/6a9b21c6158e9b429f961b810ac0730b.jpg',
			...
			setImages(images) {
				...
				images.forEach(image => {
					newImages.push(image.image_path)
				})
				return newImages
			},
			// 组件返回的type;
			...
			goPay() {
				if (this.isLogin) {
					// console.log('this.buyType', this.buyType);
					// this.buyType = 'buy';
					// this.showSku = true;

					let that = this;
					let pay = this.goodsInfo
					console.log('pay', pay);
					pay.user_id = this.userInfo.user_id
					pay.buy_count = 1;
					pay.checkin_time = '2023-04-06 12:22:22';
					pay.checkout_time = '2023-04-06 12:22:22';
					pay.remark = '';
					pay.goods_type = this.dataType
					pay.num = 1
					that.$http('order.createOrder', pay).then(res => {
						console.log('goods.pay', res);
						if (res.code === 200) {
							that.$u.toast('预定成功!');
						}
						// that.$u.toast(res.msg);
			...
			isFavorite() {
				let that = this;
				that.$http('goods.isfavorite', {
					goods_id: this.goodsInfo.homestay_id,
					user_id: that.userInfo.user_id,
					goods_type: that.dataType,
				}).then(res => {
					console.log('goods.isfavorite', res);
					if (res.code === 200) {
						// that.goodsInfo.favorite = res.data;
						that.favorite = res.data;
						// console.log('that.goodsInfo.favorite', that.goodsInfo.favorite);
					}
					// that.$u.toast(res.msg);
				});
			},
			...
			onFavorite() {
				...
				that.$http('goods.favorite', {
					goods_id: that.goodsInfo.homestay_id,
					user_id: that.userInfo.user_id,
					goods_type: that.dataType,
				}).then(res => {
					console.log('goods.favorite', res);
					if (res.code === 200) {
						// that.goodsInfo.favorite = res.data;
						that.isFavorite()
						// that.$u.toast(res.msg);
